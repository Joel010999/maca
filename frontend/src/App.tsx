...
import { 
  ...
  Target, Activity, Star, Printer, Trash2
} from 'lucide-react';
...
function App() {
  ...
  const handleDeleteBlacksmithOrder = (id: string) => {
    if (window.confirm('¿Estás seguro de eliminar este pedido al herrero?')) {
      setBlacksmithOrders(prev => prev.filter(bo => bo.id !== id));
    }
  };
    ...
    return (
      <div style={{ minHeight: '100vh', width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', background: '#f8f8f8' }}>
        ...
        <div className="glass glass-modal" style={{ maxWidth: 400, width: '100%', padding: 48, borderRadius: 40, zIndex: 10 }}>
          <h1 className="notranslate" translate="no" style={{ textAlign: 'center', marginBottom: 32, fontFamily: 'Playfair Display' }}>Male Style</h1>
  ...
  const SidebarContent = () => (
    <>
      <h2 className="notranslate" translate="no" style={{ marginBottom: 48, fontFamily: 'Playfair Display' }}>Male Style</h2>
    ...
    <div className="app-container" style={{ minHeight: '100vh', display: 'flex', position: 'relative', background: '#f8f8f8' }}>
      ...
      <header className="glass no-print show-mobile" style={{ display: 'none', padding: '15px 20px', position: 'sticky', top: 0, zIndex: 50, marginBottom: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 className="notranslate" translate="no" style={{ margin: 0, fontFamily: 'Playfair Display', fontSize: '20px' }}>Male Style</h2>
                  ...
                  <tbody>
                    {blacksmithOrders.filter(bo => bo.customer?.toLowerCase().includes(searchTerm.toLowerCase())).map(bo => (
                      <tr key={bo.id} style={{ borderBottom: '1px solid #f0f0f0', cursor: 'pointer' }} onClick={() => setViewingOrder({...bo, type: 'herreria'})}>
                        ...
                        <td style={{ padding: 20, textAlign: 'right' }}>
                          <div style={{display:'flex', gap:10, justifyContent:'flex-end'}}>
                            <button onClick={(e) => { e.stopPropagation(); setViewingOrder({...bo, type: 'herreria'}); }} style={{ padding: 10, borderRadius: 12, background: '#f5f5f5', border: 'none', cursor: 'pointer' }}><Eye size={16}/></button>
                            <button onClick={(e) => { e.stopPropagation(); handleDeleteBlacksmithOrder(bo.id); }} style={{ padding: 10, borderRadius: 12, background: '#fff1f1', color: '#ff4d4d', border: 'none', cursor: 'pointer' }}><Trash2 size={16}/></button>
      ...
      {viewingOrder && viewingOrder.customerName && (
        <div className="print-only" style={{ padding: '30px', border: '2px solid #000', width: '9cm', minHeight: '12cm', margin: '0 auto', fontFamily: 'Arial, sans-serif' }}>
          <h2 className="notranslate" translate="no" style={{ textAlign: 'center', borderBottom: '2px solid #000', paddingBottom: '10px', marginBottom: '20px' }}>MALE STYLE</h2>
